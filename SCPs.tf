...
// This SCP prevents the deletion of the WizAccess-Role IAM role
data "aws_iam_policy_document" "prevent_wizaccess_role_deletion" {
  statement {
    sid    = "PreventWizAccessRoleDeletion"
    effect = "Deny"
...
resource "aws_organizations_policy" "require_ami" {
  name        = "Prevent WizAccess-Role Deletion"
  description = "Prevent deletion of the WizAccess-Role IAM role"
  type        = "SERVICE_CONTROL_POLICY"
  content     = data.prevent_wizaccess_role_deletion.json
}